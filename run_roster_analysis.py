...
Description: Full fantasy baseball roster analysis — batter/pitcher stats, batting order,
             weakness identification (flag-based + z-score), FA scanning, dual-method
             replacement recommendations (8A position-matched, 8B position-agnostic,
             z-score based per flagged player), and signal-based add candidates
             cross-referencing the Idea 16 waiver watchlist and Idea 17 multi-method
             confidence scores.
Source Data: data-lake/01_Bronze/fantasy_baseball/ CSVs + ESPN API.
             Signal section reads <YEAR>_espn_waiver_watchlist.csv (Idea 16) and
             <YEAR>_local_multi_method_watchlist.csv (Idea 17, generated by Step 7b).
...
# ── Signal-Based Add Candidates (Ideas 16 + 17) ───────────────────────────────
_waiver_path = os.path.join(BASE, f'{YEAR}_espn_waiver_watchlist.csv')
_mm_path     = os.path.join(BASE, f'{YEAR}_local_multi_method_watchlist.csv')

_i16_by_pid = {}
if os.path.exists(_waiver_path):
    with open(_waiver_path, encoding='utf-8') as _f:
        for _r in csv.DictReader(_f):
            if _r.get('date') == TODAY:
                _i16_by_pid[_r['player_id']] = _r

_mm_rows = []
if os.path.exists(_mm_path):
    with open(_mm_path, encoding='utf-8') as _f:
        _all_mm = list(csv.DictReader(_f))
    if _all_mm:
        _mm_latest = max(r['as_of_date'] for r in _all_mm)
        _mm_rows = [r for r in _all_mm if r['as_of_date'] == _mm_latest]

_seen_sig_pids = set()
_signal_adds   = []
for _r in _mm_rows:
    _pid   = _r['player_id']
    _score = int(_r.get('confidence_score') or 0)
    _i16   = int(_r.get('passes_idea16')    or 0)
    _i16r  = _i16_by_pid.get(_pid, {})
    if _score >= 3 or _i16:
        _signal_adds.append({
            'player_name': _r['player_name'],
            'player_pos':  _r.get('player_position', ''),
            'player_type': _r['player_type'],
            'pct_owned':   float(_r.get('pct_owned') or 0),
            'score':       _score,
            'passes_i16':  _i16,
            'methods':     _r.get('methods_fired', '').replace('|', ', '),
            'i16_signals': _i16r.get('firing_signals', '').replace('|', ', '),
        })
        _seen_sig_pids.add(_pid)

# Include Idea 16-only players (>=2 signals) not already in the multi-method pool
for _pid, _r in _i16_by_pid.items():
    if _pid not in _seen_sig_pids and int(_r.get('signals_fired') or 0) >= 2:
        _signal_adds.append({
            'player_name': _r['player_name'],
            'player_pos':  _r.get('player_pos', ''),
            'player_type': _r.get('b_or_p', ''),
            'pct_owned':   float(_r.get('pct_owned') or 0),
            'score':       0,
            'passes_i16':  1,
            'methods':     '',
            'i16_signals': _r.get('firing_signals', '').replace('|', ', '),
        })

_signal_adds.sort(key=lambda x: (-x['passes_i16'], -x['score'], x['pct_owned']))
_sig_batters  = [p for p in _signal_adds if p['player_type'] == 'batter']
_sig_pitchers = [p for p in _signal_adds if p['player_type'] == 'pitcher']

lines.append('\n## Signal-Based Add Candidates (Ideas 16 & 17)\n')
lines.append(
    '> **Score (0–8):** how many of Idea 17\'s independent methods flag this player as a breakout candidate '
    '(robust-core methods A/E/F/I show cross-season lift ≥ 1.35). '
    '**I16:** passes the Idea 16 single-feature supervised threshold (ops_14d / k9_14d). '
    'Showing score ≥ 3 or I16 pass. Ranked by I16 → score → ownership.\n'
)
if not _mm_rows:
    lines.append('> _Multi-method watchlist not yet generated — run Step 7b (`watch_multi_method_signals.py`) to populate._\n')

for _label, _grp in [('Batters', _sig_batters[:15]), ('Pitchers', _sig_pitchers[:15])]:
    if not _grp:
        continue
    lines.append(f'\n### {_label}\n')
    lines.append('| Player | Pos | Own% | Score | I16 | Methods (I17) | Signals (I16) |')
    lines.append('|---|---|---|---|---|---|---|')
    for _p in _grp:
        lines.append(
            f"| {_p['player_name']} | {_p['player_pos']} | {_p['pct_owned']:.1f}% "
            f"| {_p['score']} | {'Y' if _p['passes_i16'] else ''} "
            f"| {_p['methods'] or '—'} | {_p['i16_signals'] or '—'} |"
        )

if not _signal_adds:
    lines.append('_No signal-based add candidates found._')
...
if _signal_adds:
    _top_sig = [p for p in _signal_adds if p['passes_i16'] and p['score'] >= 3][:2]
    if _top_sig:
        lines.append(f'- **Highest-confidence signal adds:** {", ".join(p["player_name"] for p in _top_sig)} (I16 pass + score ≥ 3)')
    elif _signal_adds:
        lines.append(f'- **Top signal adds:** {", ".join(p["player_name"] for p in _signal_adds[:2])}')
lines.append(f'- **Methods:** 8A/8B use pace projections vs absolute thresholds; Z-Score recs rank vs all league players; Signal adds use Ideas 16/17 breakout indicators')
